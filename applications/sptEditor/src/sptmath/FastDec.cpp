...
FastDec::FastDec(const string& value)
{
	...
	try
	{
		...
		uint64_t fractional = lexical_cast<uint64_t>(fractionalStr) * pow(10, zeros != string::npos ? zeros + 1: 0);

		_value = decimal * 1000 + fractional;
	}
	catch(bad_lexical_cast&)
	{
		raise runtime_error(str(format("FastDec::FastDec(value): Value \"%s\" cannot be converted to decimal number") % value));
	};
};

std::string FastDec::str() const
{
	return str(format("%d.%04d") % (_value / 1000) % (_value % 1000));
};

std::string FastDec::repr() const
{
	return str(format("FastDec(\"%d.%04d\")") % (_value / 1000) % (_value % 1000));
};
